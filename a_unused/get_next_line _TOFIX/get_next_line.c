...
/*   Updated: 2024/04/23 16:04:48 by myakoven         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strjoinbuff(char *line, char const *buff)
{
	size_t	len_line;
	size_t	lenbuff;
	size_t	i;
	...
	i = 0;
	j = 0;
	string = malloc(sizeof(char) * (len_line + BUFFER_SIZE) + 1);
	if (!(string))
	{
		free(line);
		return (NULL);
...
char	*ft_read(int fd, char *line, char *buffer)
{
	...
	while (bytes_read > 0)
	{
		...
		if (bytes_read < BUFFER_SIZE && bytes_read >= 0)
			ft_bzero(buffer + bytes_read, BUFFER_SIZE - bytes_read);
